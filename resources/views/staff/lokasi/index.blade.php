...
<body class="hold-transition sidebar-mini">
                            ...
                            <div class="container">
                                <button style="margin-bottom: 20px; width: 150px; height: 40px; border-radius: 20px;"
                                    data-toggle="modal" data-target="#myModal" class="btn btn-info"><span
                                        class="glyphicon glyphicon-plus"></span>Tambah Lokasi</button>
                                <table id="datakategoribarang" class="table table-striped table-bordered"
                                    style="width:100%">
                                            ...
                                            <tr>
                                                ...
                                                <td style="display: flex; justify-content: center; align-items: center;">
                                                    <div style="display: flex; gap: 8px;">
                                                        <form method="get" action="{{ route('lokasibarang.staff', ['idlokasi' => $items->idlokasi]) }}">
                                                            <button type="submit" class="btn btn-primary">
                                                                <i class="fas fa-box"></i> Daftar Barang
                                                            </button>
                                                        </form>
                                                        <button data-toggle="modal" data-target="#modaledit_{{ $items->idlokasi }}" class="btn btn-warning">
                                                            <i class="fas fa-edit"></i>
                                                        </button>
                                                        <button class="btn btn-danger" data-toggle="modal" data-target="#modaldelete_{{ $items->idlokasi }}">
    ...
    @if (!empty($items))
        @foreach ($data_lokasi as $items)
            <!-- Modal Edit -->
            <div id="modaledit_{{ $items->idlokasi }}" class="modal fade">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form method="post"
                                action="{{ route('lokasi.update', ['idlokasi' => $items->idlokasi]) }}" novalidate>
                                @csrf
                                @method('PUT')

                                <div class="form-group">
                                    <label for="edit-nama-lokasi" class="modal-label">Nama Lokasi</label>
                                    <input type="text" id="edit-nama-lokasi" name="namalokasi"
                                        value="{{ $items->namalokasi }}" class="form-control" required>
                                </div>
                                <div class="form-group">
                                    <label for="edit-created-at" class="modal-label">Tanggal Dibuat</label>
                                    <input type="datetime-local" id="edit-created-at" name="created_at"
                                        value="{{ \Carbon\Carbon::parse($items->created_at)->format('Y-m-d\TH:i') }}"
            ...
            <!-- Modal Delete -->
            <div class="modal fade" id="modaldelete_{{ $items->idlokasi }}" tabindex="-1" role="dialog"
                aria-labelledby="modaldelete_{{ $items->idlokasi }}Label" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="modaldelete_{{ $items->idlokasi }}Label">Konfirmasi penghapusan</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            Anda yakin ingin menghapus data lokasi ini?
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-primary" data-dismiss="modal">Cancel</button>
                            <form action="{{ route('lokasi.hapus', ['lokasi' => $items->idlokasi]) }}" method="POST"
                                class="inline-block">
                                @csrf
                                @method('DELETE')
    ...
    <div id="myModal" class="modal fade">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">MASUKAN DATA LOKASI</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                </div>
                <div class="modal-body">
                    <form method="post" id="myForm" action="{{ route('lokasi.post') }}" novalidate>
                        @csrf
