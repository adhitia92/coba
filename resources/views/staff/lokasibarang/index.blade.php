...
<head>
    ...
    <link rel="stylesheet" type="text/css"
        href="https://cdn.datatables.net/responsive/2.2.10/css/responsive.dataTables.min.css" />
    <script type="text/javascript" src="https://code.jquery.com/jquery-3.6.4.min.js"></script>
    <script type="text/javascript" src="https://cdn.datatables.net/v/bs4/dt-1.13.7/datatables.min.js"></script>
    <script type="text/javascript" src="https://cdn.datatables.net/responsive/2.2.10/js/dataTables.responsive.min.js">
    </script>
...
<body class="hold-transition sidebar-mini">
    ...
    <div class="wrapper">
        ...
        <nav class="main-header navbar navbar-expand navbar-white navbar-light">
            ...
            <ul class="navbar-nav">
                ...
                <li class="nav-item d-none d-sm-inline-block">
                    <a href="{{ route('lokasi.staff') }}" class="nav-link">Daftar Lokasi</a>
                </li>
            </ul>
                    ...
                    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu"
                        ...
                        <li class="nav-item">
                            <a href="{{ route('lokasi.staff') }}"
                                class="nav-link {{ Request::segment(1) === 'lokasibarang' ? 'active' : '' }}">
            ...
            <section class="content">
                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Data Data Barang</h3>
                        ...
                        <div class="box-body">
                            <div class="container">
                                ...
                                <table id="datatabel" class="table table-striped table-bordered" style="width:100%">
                                    ...
                                    <tbody>
                                        <?php $no = 1; ?>
                                        @foreach ($data_lokasibarang as $items)
                                            <tr>
                                                <td>{{ $no++ }}</td>
                                                <td>{{ $items->namalokasi }}</td>
                                                <td>( {{ $items->idbarang }} ) {{ $items->namakategori }}
                                                ...
                                                <td>
                                                    <div
                                                        ...
                                                        <button class="btn btn-danger" data-toggle="modal"
                                                            data-target="#modaldelete_{{ $items->idlokasibarang }}">
    ...
    <!-- modal-->
    @foreach ($data_lokasibarang as $item)
        <div class="modal fade" id="modaldelete_{{ $item->idlokasibarang }}" tabindex="-1" role="dialog"
            aria-labelledby="modaldelete_{{ $item->idlokasibarang }}Label" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="modaldelete_{{ $item->idlokasibarang }}Label">Konfirmasi
                            Penghapusan</h5>
                    ...
                    <div class="modal-footer">
                        ...
                        <form action="{{ route('lokasibarang.hapus', ['idlokasibarang' => $item->idlokasibarang]) }}"
    ...
    <div id="myModal" class="modal fade">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="post" id="myForm" action="{{ route('lokasibarang.post') }}" novalidate>
                        @csrf

                        <input type="hidden" name="idkategori" id="idkategori"
                            value="{{ $item->kategori->idkategori ?? '' }}"> <!-- Ensure this line is present -->

                        <div class="form-group">
                            <label class="modal-label" for="idlokasi">Pilih Lokasi</label>
                            <select name="idlokasi" id="idlokasi" class="form-control" required>
                                <option value="">Pilih Lokasi</option>
                                @foreach ($lokasilist as $lokasi)
                                    <option value="{{ $lokasi->idlokasi }}" data-idlokasi="{{ $lokasi->idlokasi }}"
                            ...
                            <select name="idbarang" id="idbarang" class="form-control" required>
                                ...
                                @foreach ($kombinasilist as $item)
                                    <option value="{{ $item->idbarang }}"
                                        ...
                                        data-namakategori="{{ $item->kategori->namakategori }}"
                                        data-jenis="{{ $item->kategori->jenis }}"
                                        data-merk="{{ $item->kategori->merk }}" data-kondisi="{{ $item->kondisi }}">
                                        ({{ $item->idbarang }})
    ...
    <script>
        $(document).ready(function() {
            ...
            // Correct the variable name to idLokasiFromUrl
            var currentUrl = window.location.href;
            var idLokasiFromUrl = extractIdLokasi(currentUrl);

            if (idLokasiFromUrl !== null) {
                $('#idlokasi').val(idLokasiFromUrl);
            }

            function extractIdLokasi(url) {
                var regex = /\/lokasibarang\/(\d+)/;
